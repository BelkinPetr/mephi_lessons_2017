...
void displayMinMax(double minLoc, double minVal, double maxLoc, double maxVal){
    cout << "minLoc = " << minLoc << ", minVal = " << minVal << endl;
    cout << "maxLoc = " << maxLoc << ", maxVal = " << maxVal << endl;
}
...
void findMaxLocByBisection(double(*func)(double), double a, double b, double eps, double deltaX,
                double& maxLoc, double& maxVal){
    double x1, x2;
    double f_x1, f_x2;
...
void findMaxLocByGoldenRatio(double(*func)(double), double a, double b, double eps,
                             double& maxLoc, double& maxVal){
    double x1, x2;
    double f_x1, f_x2;
...
int main()
{
    ...
    findMaxLocByGoldenRatio(func, 2.0, 4.0, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = sin(x), a = 0, b = 6.28" << endl;
    findMinLocByGoldenRatio(func1, 0.0, 6.28, 0.01, minLoc, minVal);
    findMaxLocByGoldenRatio(func1, 0.0, 6.28, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = cos(x), a = -1.57, b = 3.14" << endl;
    findMinLocByGoldenRatio(func2, -1.57, 3.14, 0.01, minLoc, minVal);
    findMaxLocByGoldenRatio(func2, -1.57, 3.14, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = abs(x), a = -10, b = 10" << endl;
    findMinLocByGoldenRatio(func3, -10, 10, 0.01, minLoc, minVal);
    findMaxLocByGoldenRatio(func3, -10, 10, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = log(x), a = 1, b = 10" << endl;
    findMinLocByGoldenRatio(func4, 1, 10, 0.01, minLoc, minVal);
    findMaxLocByGoldenRatio(func4, 1, 10, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = exp(x), a = 0, b = 10" << endl;
    findMinLocByGoldenRatio(func5, 0, 10, 0.01, minLoc, minVal);
    findMaxLocByGoldenRatio(func5, 0, 10, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = atan(x), a = -1, b = 1" << endl;
    findMinLocByGoldenRatio(func6, -1.0, 1.0, 0.01, minLoc, minVal);
    findMaxLocByGoldenRatio(func6, -1.0, 1.0, 0.01, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = (x - 1)^2 - 1, x = {0, ..., 2}, T = 2, a = -4, b = 4" << endl;
    findMinLocByGoldenRatio(func7, -4, 4, eps, minLoc, minVal);
    findMaxLocByGoldenRatio(func7, -4, 4, eps, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = log2(x + 1),  a = 0, b = 10" << endl;
    findMinLocByGoldenRatio(func8, 0, 10, eps, minLoc, minVal);
    findMaxLocByGoldenRatio(func8, 0, 10, eps, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);

    cout << "f = 2^x,  a = 2, b = 8" << endl;
    findMinLocByGoldenRatio(func9, 2, 8, eps, minLoc, minVal);
    findMaxLocByGoldenRatio(func9, 2, 8, eps, maxLoc, maxVal);
    displayMinMax(minLoc, minVal, maxLoc, maxVal);
